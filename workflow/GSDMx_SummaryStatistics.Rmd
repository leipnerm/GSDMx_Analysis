...
matlab_dir <- "./../analysis/MATLAB_data"
...
```{r Height Profiles, echo=FALSE, eval = FALSE}
# NOTE 28.04.2021: Code left for future modification/use, however disable in current iteration via "eval = FALSE" above

##################################
# Plotting 16                    #
##################################

# Update 20200604: Replace pore #6 with version from image 13 to have greater clearance from left side of image
pores16$X[[1]] <- pores13$X[[2]]
pores16$CS[[5]] <- pores13$CS[[10]] # recall, there are 5 CS lines per object

# Collect list of 2D single oligos from original report
images_16 <- list.files(paste(matlab_dir,"G61A3-310120_016_14_REPORT/images/",sep=""),"*2D.png",full.names = T)
images_16_num <- as.integer(unlist(strsplit(gsub("^.*/","",images_16),split="_"))[6*(1:length(images_16))-1])

df_images_16 <- data.frame(num = images_16_num, image = images_16,stringsAsFactors=FALSE)
df_images_16 <- df_images_16[order(df_images_16$num),]

# Oligomers to plot cross sections of from Height_G61A3-310120_016_16.tsv
oligomers_016_16 <- c(1,6,8,10,22,26)

df16 <- data.frame(x=double(),
                   y=double(),
                   zero=double(),
                   id=integer())
for(i in oligomers_016_16){
  # Extract data from matlab data file
  x <- pores16$X[[i]][[1]][1,]
  y <- pores16$CS[[5+(i-1)*5]][[1]][1,]
  
  x <- x - min(x)
  
  c_df <- data.frame(x=x,y=y,zero=0,id=i)
  
  # Save to dataframe
  df16 <- rbind(df16,c_df)
  
  
  ggplot(c_df) +
    My_Theme +
    geom_line(color="black",size=1.5,linetype="dashed",aes(x=x,y=zero)) +
    geom_line(color="red",size=3,aes(x=x,y=y)) +
    # geom_text(aes(x = Inf, y = Inf, label = id),
    #           size = 16,
    #           hjust = 1.2,
    #           vjust = 1.3,
    #           inherit.aes = FALSE) +
    geom_segment(aes(x = 47, xend = 67,y = -4, yend = -4),size=2) +
    ylab("Height (nm)") +
    ...
    scale_y_continuous(limits=c(-5,4), breaks=c(-3,0,3), expand = c(0, 0))
  
  ggsave(filename = paste(outdir,"","oligomers_016_16-",
                          i,".png",sep=""),
         width = 12, height = 8,units = "cm")
  
  # Copy 2D figure to paper figure directory
  file.copy(from=df_images_16[i,2], to=paste(outdir,"oligomers_016_16_",i,"_2D.png",sep=""), 
            overwrite = TRUE, recursive = FALSE, 
            copy.mode = TRUE)
  
}

print("oligomers_016_16")

forComposite16 <- oligomers_016_16

p_df16 <- df16 %>%
  filter(id %in% forComposite16) %>%
  ggplot() +
  geom_line(color="black",size=1.5,linetype="dashed",aes(x=x,y=zero)) +
  geom_line(color="red",size=3,aes(x=x,y=y)) +
  # geom_text(aes(x = Inf, y = Inf, label = id),
  ...
  theme(axis.text.y = element_text(size=ymarker_size),
        axis.title.y = element_text(size=ylabel_size),
        plot.margin = margin(t=5,b=30,r=5, l=23)) +
  facet_wrap( ~ id, ncol=3) +
  theme(strip.text.x = element_blank())

print(p_df16)

ggsave(filename = paste(outdir,"","oligomers_016_16-composite.png",sep=""),
...
labelList <- sapply(strsplit(fileList,"\\.tsv"),'[',1) %>%
  {sapply(strsplit((.),"Height_"),'[',2)} %>%
  {sub('[_][^_]+$', '',(.))}
...
# Proprosed whole figure 1
library(gridExtra)

g3 <- arrangeGrob(g,p_bar_height,
                  layout_matrix = c(1,1,1,2),
                  padding = 1)

ggsave(g3, filename = paste(outdir,"","Figure_All_Composite.png",sep=""),
       width = 68 , height = 32,units = "cm")
